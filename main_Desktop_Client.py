...
from Share.b64 import *
from Share.const import *
from Share.message import *
from Share.timetips import *
from Share.notification import *
from Share.HEX import *
from Share.hwid import *
...
tkt.IconButton(cv, (20, 20), text="IconButton", image=tkt.PhotoImage(file=os.path.join(directory_path,"Resource","Settings.png")).resize(32, 32))
# 存储InputBox实例
BackButton = tkt.Button(cv, (20, -200), text="<-返回", command=lambda:BackMainMenu())
...
def login_test(username_input, password_input):
    ...
    try:
        ...
        if 'Login' in config.sections() :
            if HEX(username) == usernameHEX and HEX(password) == passwordHEX :
                if not hwid == "" :
                    if hwid == b64(config.get('Login', 'hwid')) :
                        timetips("登录成功")
                        send_notification("github.com/Rain-dewdrop/Login-System", "登录成功,欢迎回来！")
                        username_b64 = b64code(username)
                        password_b64 = b64code(password)
                        hwid_b64 = b64code(hwid)
                        config['Login'] = {}
                        ...
                        timetips("配置文件hwid验证失败")
                        message("Hwid验证失败,请不要复制他人的配置文件,请在删除配置文件后重试!")
                else :
                    timetips("登录成功！")
                    send_notification("github.com/Rain-dewdrop/Login-System", "登录成功,欢迎回来！")
                    username_b64 = b64code(username)
...
if config.read(config_file_path):
    ...
    if 'Login' in config.sections() and config.get('Login', 'username') != "" and config.get('Login', 'password') != "" and config.get('Login', 'hwid') != "":
        ...
        timetips("成功解密配置文件")
        password = b64(password_b64)
        username = b64(username_b64)
